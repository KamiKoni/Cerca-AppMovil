...
import { useListingDetail, useUpdateListing } from '../infrastructure/query/hooks';
...
export function ListingEditScreen({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    if (listing.pricing.model === 'fixed') {
      ...
      setHourlyAmount((listing.pricing.hourlyRate.amountMinor / 100).toString());
      setMinimumHours(listing.pricing.minimumHours.toString());
    } else {
      setStartingAmount(listing.pricing.startingFrom ? (listing.pricing.startingFrom.amountMinor / 100).toString() : '');
    }
  }, [detailQuery.data]);
        ...
        model === 'fixed'
          ? {
              ...
              price: {
                ...
                currency: listing.pricing.model === 'fixed' ? listing.pricing.price.currency : 'USD',
              },
            }
          : model === 'hourly'
          ? {
              ...
              hourlyRate: {
                ...
                currency: listing.pricing.model === 'hourly' ? listing.pricing.hourlyRate.currency : 'USD',
              },
          ...
          : {
              ...
              startingFrom: startingAmount
                ? {
                    ...
                    currency: listing.pricing.model === 'quote' && listing.pricing.startingFrom
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <TextInput value={categoryId} onChangeText={setCategoryId} style={styles.input} />
      ...
      <View style={styles.modelRow}>
        {(['fixed', 'hourly', 'quote'] as const).map((option) => (
          <Pressable
            key={option}
            style={[styles.modelChip, model === option && styles.modelChipActive]}
            onPress={() => setModel(option)}
          >
            <Text style={[styles.modelChipText, model === option && styles.modelChipTextActive]}>
