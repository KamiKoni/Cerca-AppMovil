...
const lastCall = (fetchFn: ReturnType<typeof fakeFetch>) =>
...
describe("createHttpClient", () => {
  ...
  it("turns an error response into a typed domain error", async () => {
    ...
    const error = await client
      .request("/listings/l1", actorSchema)
      .catch((e: unknown) => e);
  ...
  it("drops absent and blank query parameters", async () => {
    ...
    await client.request(
      "/listings",
      z.object({ items: z.array(z.unknown()), nextCursor: z.null() }),
      {
        query: {
          lat: 6.24,
          lng: -75.58,
    ...
    expect(lastCall(fetchFn)[0]).toBe(
      "http://localhost:3333/v1/listings?lat=6.24&lng=-75.58&radiusKm=10",
    );
  });
