...
describe('toApiError', () => {
  ...
  it('distinguishes the two 422s the data layer has to act on differently', () => {
    ...
    expect(toApiError(422, problem422Idempotency).code).toBe('IDEMPOTENCY_KEY_REQUIRED');
  ...
  it('documents why this schema is not the shared one', () => {
    ...
    const viaShared = sharedProblemSchema.parse(problem403) as Record<string, unknown>;
