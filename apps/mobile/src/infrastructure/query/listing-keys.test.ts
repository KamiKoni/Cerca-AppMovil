...
describe('listingKeys', () => {
  ...
  it('lets one invalidation reach every search', () => {
    ...
    expect(isPrefixOf(searches, listingKeys.search({ query: 'plumber' }))).toBe(true);
    expect(isPrefixOf(searches, listingKeys.search({ categoryId: 'cat-1' }))).toBe(true);
  ...
  it('does not let a detail key match the searches prefix', () => {
    expect(isPrefixOf(listingKeys.searches(), listingKeys.detail('abc'))).toBe(false);
  });
  ...
  it('produces one key for a micro-movement of the map', () => {
    ...
    const a = listingKeys.search({ coords: { lat: 19.432608, lng: -99.133209 } });
    const b = listingKeys.search({ coords: { lat: 19.432611, lng: -99.13321 } });
  ...
  it('still separates searches that are genuinely far apart', () => {
    const medellin = listingKeys.search({ coords: { lat: 6.2442, lng: -75.5812 } });
    const bogota = listingKeys.search({ coords: { lat: 4.711, lng: -74.0721 } });
  ...
  it('trims the query but keeps its case', () => {
    expect(listingKeys.search({ query: '  plumber ' })).toEqual(listingKeys.search({ query: 'plumber' }));
    ...
    expect(listingKeys.search({ query: 'Plumber' })).not.toEqual(listingKeys.search({ query: 'plumber' }));
  });

  it('separates searches that differ only by radius', () => {
    const near = listingKeys.search({ coords: { lat: 6.24, lng: -75.58 }, radiusKm: 5 });
    const wide = listingKeys.search({ coords: { lat: 6.24, lng: -75.58 }, radiusKm: 20 });
  ...
  it('ignores the order the filters were written in', () => {
    const a = listingKeys.search({ query: 'plumber', categoryId: 'cat-1', radiusKm: 5 });
    const b = listingKeys.search({ radiusKm: 5, categoryId: 'cat-1', query: 'plumber' });
