...
import { guardDecision, HOME_ROUTE, SIGN_IN_ROUTE, type RouteGroup } from './route-guard';
...
describe('guardDecision', () => {
  it('waits in every group while the keystore is still being read', () => {
    ...
    for (const group of GROUPS) {
      expect(guardDecision(BOOTSTRAPPING, group)).toEqual({ type: 'bootstrap' });
    }
  });

  it('sends the launch route to the area the session earns', () => {
    expect(guardDecision(AUTHENTICATED, 'entry')).toEqual({ type: 'redirect', to: HOME_ROUTE });
    expect(guardDecision(ANONYMOUS, 'entry')).toEqual({ type: 'redirect', to: SIGN_IN_ROUTE });
  });
  ...
  it('keeps a signed-in user out of the sign-in screen', () => {
    ...
    // having failed.
    expect(guardDecision(AUTHENTICATED, 'auth')).toEqual({ type: 'redirect', to: HOME_ROUTE });
    expect(guardDecision(ANONYMOUS, 'auth')).toEqual({ type: 'render' });
  });

  it('lets only an authenticated session through a protected group', () => {
    expect(guardDecision(AUTHENTICATED, 'protected')).toEqual({ type: 'render' });
    expect(guardDecision(ANONYMOUS, 'protected')).toEqual({ type: 'redirect', to: SIGN_IN_ROUTE });
  });
